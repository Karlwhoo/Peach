                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                @foreach ($invoices as $invoice)
                                    <tr>
                                        <td>{{ $invoice->id }}</td>
                                        <td>{{ $invoice->guest->Fname . ' ' . $invoice->guest->Mname . ' ' . $invoice->guest->Lname ?? 'No Guest' }}</td>
                                        <td>{{ $invoice->PaymentMethod }}</td>
                                        <td>{{ $invoice->SubTotal }}</td>
                                        <td>{{ $invoice->TaxTotal }}</td>
                                        <td>{{ $invoice->Total }}</td>
                                        <td>
                                            <a href="/invoice/{{ $invoice->id }}/restore" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Restore">
                                                <i class="fa-solid fa-undo ml-2 text-success"></i>
                                            </a>
                                            <a href="/invoice/{{ $invoice->id }}/parmanently/delete" data-bs-toggle="tooltip" data-bs-placement="bottom" title="Permanently Delete">
